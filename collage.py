...
print('Loading images & Calculating averages...',end='\r')
...
target_avgs = tuple( tuple( tuple( map(int, y) ) for y in x ) for x in cv2.resize(cv2.imread(target_name), (n_cells, n_cells)) )
output = np.zeros((output_size, output_size, 3), dtype=np.uint8)
print('Loading images & Calculating averages... Done')
...
for i in range(n_cells):
    for j in range(n_cells):
        ...
        output[i*cell_size:(i+1)*cell_size, j*cell_size:(j+1)*cell_size] = inputs[min_p]
